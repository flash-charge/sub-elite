...
function normalizeClientModel(model) {
  return {
    ...
    general: {
      ...
      lanAllowedIps: model?.general?.lanAllowedIps || [],
      lanDisallowedIps: model?.general?.lanDisallowedIps || [],
      authentication: model?.general?.authentication || [],
      skipAuthPrefixes: model?.general?.skipAuthPrefixes || [],
    ...
    dns: {
      ...
      fakeIpFilter: model?.dns?.fakeIpFilter || ['*.lan', '*.local'],
      defaultNameserver: model?.dns?.defaultNameserver || ['1.1.1.1', '8.8.8.8'],
      nameserver: model?.dns?.nameserver || ['https://dns.google/dns-query', 'https://cloudflare-dns.com/dns-query'],
      fallback: model?.dns?.fallback || [],
      fallbackFilter: model?.dns?.fallbackFilter || {},
      directNameserver: model?.dns?.directNameserver || [],
      directNameserverFollowPolicy: Boolean(model?.dns?.directNameserverFollowPolicy),
      proxyServerNameserver: model?.dns?.proxyServerNameserver || [],
    ...
    tun: {
      ...
      dnsHijack: model?.tun?.dnsHijack || ['any:53'],
      mtu: model?.tun?.mtu || 0,
      ...
      routeAddressSet: model?.tun?.routeAddressSet || [],
      routeExcludeAddressSet: model?.tun?.routeExcludeAddressSet || [],
      routeAddress: model?.tun?.routeAddress || [],
      routeExcludeAddress: model?.tun?.routeExcludeAddress || [],
      includeInterface: model?.tun?.includeInterface || [],
      excludeInterface: model?.tun?.excludeInterface || [],
      includeUid: model?.tun?.includeUid || [],
      includeUidRange: model?.tun?.includeUidRange || [],
      excludeUid: model?.tun?.excludeUid || [],
      excludeUidRange: model?.tun?.excludeUidRange || [],
      includeAndroidUser: model?.tun?.includeAndroidUser || [],
      includePackage: model?.tun?.includePackage || [],
      excludePackage: model?.tun?.excludePackage || [],
    },
    ntp: {
    ...
    ruleProviders: Array.isArray(model?.ruleProviders)
      ? model.ruleProviders.map((provider) => ({ ...provider, target: provider.target || 'PROXY' }))
      : [],
    proxyProviders: Array.isArray(model?.proxyProviders) ? model.proxyProviders : [],
    ...
    groups: Array.isArray(model?.groups) ? model.groups.map((group) => ({ ...group, proxies: group.proxies || [], use: group.use || [] })) : [],
    rules: Array.isArray(model?.rules) ? model.rules : ['MATCH,PROXY'],
  }
}
