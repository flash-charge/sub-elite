...
export function renderProtocolFields(proxy) {
  ...
  if (type === 'hysteria2') {
    ...
    const advancedFields = [
      `<label><span>BBR Profile</span><select data-field="bbr-profile">${renderSelectOptions(selectOptionsWithCurrent(hysteria2BbrProfileOptions, proxy['bbr-profile'] || ''), proxy['bbr-profile'] || '')}</select></label>`,
      `<label><span>Obfs</span><select data-field="obfs">${renderSelectOptions(selectOptionsWithCurrent(hysteria2ObfsOptions, proxy.obfs || ''), proxy.obfs || '')}</select></label>`,
      `<label><span>Obfs Password</span><input type="text" data-field="obfs-password" value="${escapeAttr(proxy['obfs-password'] || '')}"></label>`,
    ]
    const realmBasicFields = [
    ...
    return `
      ${renderOptionDetails('Hysteria2', basicFields, advancedFields)}
      ${renderOptionDetails('realm-opts', realmBasicFields, realmAdvancedFields)}
    `
  }
  if (type === 'ss') {
    return `
...
export function renderSelectOptions(options, value) {
  return options
    .map((option) => {
      const optionValue = selectOptionValue(option)
      const optionLabel = isPlainObject(option) ? (option.label ?? optionValue) : (optionValue || 'default')
...
function selectOptionsWithCurrent(options, value) {
  return value && !options.some((option) => selectOptionValue(option) === value) ? [value, ...options] : options
}

function selectOptionValue(option) {
  if (isPlainObject(option)) return String(option.value ?? '')
